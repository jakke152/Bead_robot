...
#include "motor.hpp"
Motor::Motor(): pin1(D13), pin2(D11), pin3(D9), pin4(D10){}
Motor::Motor(PinName pinName1, PinName pinName2, PinName pinName3,
             PinName pinName4)
    : pin1(pinName1), pin2(pinName2), pin3(pinName3), pin4(pinName4) {}
...
int Motor::Speed(float speedA, float speedB) {
  ...
  // If speed values have passed the checks above then the following code will
  // be executed

  if (speedA < 0.0f) { // Reverse A motor
    this->pin1 = -speedA;
    this->pin2 = 0.0f;
  }

  else { // Forward A motor
    this->pin1 = 0.0f;
    this->pin2 = speedA;
  }

  if (speedB < 0.0f) { // Reverse B motor
    this->pin3 = -speedB;
    this->pin4 = 0.0f;
  } else { // Forward B motor
    this->pin3 = 0.0f;
    this->pin4 = speedB;
  }
